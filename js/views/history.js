...
import { addDays, weekday, stats, challenge, historyStart, monthRates, weekdayRates, streaks, WEEKDAYS } from '../logic.js';
import { esc, rgba, fmt, plural } from '../ui.js';
import { heatCell } from './today.js';
...
export function render(ctx, root, id) {
  ...
  const months = monthRates(data, h, from, today);
  const overall = months.reduce((a, m) => ({ due: a.due + m.due, done: a.done + m.done }), { due: 0, done: 0 });
  const wdays = weekdayRates(data, h, from, today);
  const top = streaks(data, h, from, today).slice(0, 5);
  const back = h.status === 'aktiv' ? '#/' : '#/routinen';

  root.innerHTML = `
    <a class="back" href="${back}">‹ ${h.status === 'aktiv' ? 'Heute' : 'Routinen'}</a>
    <section class="card">
      <div class="top">
      ...
      <div class="meta">
        ...
        <span>Ziel <b>${h.target}</b> pro Tag</span>
        <span>${h.weekdays && h.weekdays.length ? 'Fällig <b>' + h.weekdays.map(i => WEEKDAYS[i]).join(' ') + '</b>' : '<b>täglich</b>'}</span>
      ...
      <div class="stats">
        ...
        <div class="stat"><b>${s.total}</b><span>erfüllte Tage</span></div>
        <div class="stat"><b>${overall.due ? Math.round(overall.done / overall.due * 100) : 0} %</b><span>Quote gesamt</span></div>
      </div>
      ${ch ? `
        <div class="challenge">
          <div class="label"><span>Challenge ab ${esc(fmt(ch.start, { day: 'numeric', month: 'short' }))}</span><span><b>${ch.done}</b> von ${ch.days} Tagen${ch.complete ? ' · geschafft' : ''}</span></div>
    ...
    <div class="section-title">Quote je Wochentag</div>
    ...
    <section class="card">${top.length ? `<ul class="streaks">${top.map(x => `
      <li><span>${esc(fmt(x.start, { day: 'numeric', month: 'short', year: 'numeric' }))} – ${esc(fmt(x.end, { day: 'numeric', month: 'short', year: 'numeric' }))}</span><b>${plural(x.length, 'Tag', 'Tage')}</b></li>`).join('')}</ul>`
      : '<div class="small">Noch keine erfüllten Tage.</div>'}</section>`;
...
/** 53 Wochen bis heute mit Monats- und Wochentagsbeschriftung. */
function yearGrid(ctx, h, host) {
  ...
  wdl.className = 'wdl';
  wdl.innerHTML = WEEKDAYS.map((n, i) => `<span>${i % 2 === 0 ? n : ''}</span>`).join('');
  ...
  grid.className = 'grid';
  for (let i = 0; i < weeks * 7; i++) grid.appendChild(heatCell(ctx, h, addDays(start, i)));
